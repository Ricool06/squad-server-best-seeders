...
const differenceBetweenDateTimeStrings = (startTimestamp, endTimestamp) => dayjs(endTimestamp).diff(dayjs(startTimestamp), 'minutes');
...
export const calculateBestSeedersOfTheWeek = async now => {
  ...
  const sessionsMatrix = await Promise.all(
    seedingPeriods.map(async seedingPeriod => got.get(sessionUrl, {
      headers,
      searchParams: new URLSearchParams({
        // "include": "player",
        ...seedingPeriod,
